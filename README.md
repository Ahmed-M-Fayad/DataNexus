...
> 📍 **Status:** core engine, CLI, and Streamlit dashboard are functional end-to-end
> against a real database — validating both flat files (CSV/JSON/Parquet) and live
> PostgreSQL tables. See [Project Status](#project-status) below for exactly what's
> built vs. planned.
...
1. **Profile** a dataset — row counts, null %, distinct counts, min/max/mean, pattern
   detection (emails, etc.) — from a CSV file *or* a live database table
...
4. **Persist** every run, every per-check result, and the dataset profile, so history
   is queryable later
5. **Visualize** quality trends, run history, and alerts in a Streamlit dashboard

The validation engine is source-agnostic: a `DataSource` row is either a file path
(csv/json/parquet) or a database connection string (postgresql/mysql). The same
profiler, validation engine, and scoring logic run against either — a CSV file and a
live Postgres table are validated through the exact same code path.

All of this is reachable through a CLI tool and a web dashboard — no extra
...
# 6. Seed demo data
datanexus seed --target csv     # sample_customers.csv demo (file-based validation)
datanexus seed --target db      # demo_employees / demo_departments Postgres tables
                                 # (real database-table validation — unique, not_empty,
                                 #  range, regex, in_set, foreign_key, freshness)
datanexus seed --target all     # both in one command
...
|---|---|
| `datanexus seed [--target csv\|db\|all] [--reset]` | Seeds demo data sources + validation configs. `csv` = file-based demo (default), `db` = live Postgres table demo, `all` = both |
...
| `datanexus sources list` | List registered data sources (CSV files and database connections) |
...
flowchart TD
    ...
    SRC["Sources: CSV/JSON/Parquet files + PostgreSQL/MySQL tables ✅"]
    DB["PostgreSQL metadata store — 8 tables, Alembic ✅"]
...
The `dataset:` value is matched against `datasets.table_name` regardless of whether
that dataset lives in a CSV file or a live database table — the config format is
identical either way. See `datanexus seed --target db` for a full example that
exercises every check type above against a real Postgres table, including a
`foreign_key` check validating `department_id` against a reference table.
...
- Data Profiler (column statistics, pattern detection) — works against CSV files
  and live database tables through the same code path
- Validation Engine (check execution, severity-weighted quality scoring) — same:
  source-agnostic, validates files and DB tables identically
...
- Streamlit Dashboard — Overview, Trends, Run Details, Data Profile, Alerts, and
  Config Explorer pages, reading live from the metadata store
- Demo seed script with deliberately corrupted sample data, for both a CSV file
  (`--target csv`) and real PostgreSQL tables (`--target db`)

### 🔜 Designed, not yet implemented

- **Alert Manager** — Slack/email dispatch on failed runs (schema exists, table is
  `alerts`, logic not written; the dashboard's Alerts page reads the schema but has
  nothing to display until dispatch logic is written)
- **REST API** (Flask) — not required by the CLI or dashboard, which both talk to
  the database directly; would matter for Airflow/CI integration later
...
│   └── dashboard/         # Streamlit dashboard (Overview/Trends/Run Details/
│                          #   Data Profile/Alerts/Config Explorer)
